...
static struct
{
    ...
    {"local6", LOG_LOCAL6},
    {"local7", LOG_LOCAL7},
    {"auth", LOG_AUTH},         //security/authorization messages (DEPRECATED Use LOG_AUTHPRIVinstead)
    ...
    {"kern", LOG_KERN},         //kernel messages (these can't be generage from user processes)
    {"lpr", LOG_LPR},           //line printer subsystem
    {"mail", LOG_MAIL},         //mail subsystem
    {"news", LOG_NEWS},         //USENET news subsystem
    {"syslog", LOG_SYSLOG},     //messages generated internally by syslogd(8)
...
static char _log_path[FILENAME_LEN]; //日志文件路径
static char _log_name[FILENAME_LEN];
static char _log_name_prefix[FILENAME_LEN]; //日志名 去除.后缀
                      ...
                      const char *func, const char *msg)
{
    ...
    {
        snprintf(s_file, sizeof(s_file), "[%s:%d]", file, line);
...
static int log_init_file(const char *ident)
{
    ...
    if (ident == NULL)
    {
        ...
        snprintf(_log_name_prefix, sizeof(_log_name_prefix), "%s", _proc_name);
...
static void log_init_once(void)
{
    ...
    UPDATE_LOG_PREFIX(_log_prefix, LOG_PIDTID_BIT);
    ...
    if (type != 0)
    { //if not stdin, set output to type
        _log_output = type;
    }
    switch (_log_output)
    {
